 ...
 */
class SensorChecker extends EventEmitter {
    /**
     * 
     * @param {Aircon} aircon 
    ...
    check() {
        this.aircon
            .sensorInfo()
            .then((sensorValues) => {
                if (!sensorValues || !sensorValues.htemp) {
                    // 取得できなかった
                } else {
                    const temperature = sensorValues.htemp;
